...
def replace_with_template(text, subtask_type):
    # Join the list elements with space
    if subtask_type == "COLLECT":
        return PATTERNS['piextract'][0].format(text=text)
    elif subtask_type == "NOT_COLLECT":
        return PATTERNS['piextract'][1].format(text=text)
    elif subtask_type == "SHARE":
        return PATTERNS['piextract'][2].format(text=text)
    else:
        return PATTERNS['piextract'][3].format(text=text)
